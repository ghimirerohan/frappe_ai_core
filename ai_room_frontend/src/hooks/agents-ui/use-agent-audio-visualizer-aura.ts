...
export function useAgentAudioVisualizerAura(
  ...
  useEffect(() => {
    ...
    const pulse = Math.max(volume, 0.08);
    if (!scaleMotionValue.isAnimating()) {
      animateScale(0.22 + 0.28 * pulse, { duration: 0 });
    }
    animateAmplitude(0.75 + 0.65 * pulse, { duration: 0 });
    animateBrightness(1.35 + 1.15 * pulse, { duration: 0 });
    animateFrequency(1.25 + 0.35 * pulse, { duration: 0 });
  }, [
    state,
    volume,
    scaleMotionValue,
