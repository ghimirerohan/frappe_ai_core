...
const AuraShader = React.forwardRef<
  HTMLDivElement,
  AuraShaderProps & ComponentPropsWithoutRef<'div'>
>(function AuraShader(
  {
    shape = 1.0,
    speed = 1.0,
...
const AURA_SIZE_PX: Record<AuraSize, number> = {
  icon: 24,
  sm: 56,
  md: 112,
  lg: 224,
...
export const AgentAudioVisualizerAuraVariants = cva(['relative aspect-square shrink-0 overflow-hidden'], {
  variants: {
    size: {
      icon: 'h-[24px] w-[24px]',
      sm: 'h-[56px] w-[56px]',
 ...
 * ```
 */
export type AgentAudioVisualizerAuraComponentProps = AgentAudioVisualizerAuraProps &
  VariantProps<typeof AgentAudioVisualizerAuraVariants> &
  ComponentPropsWithoutRef<'div'>;

export const AgentAudioVisualizerAura = React.forwardRef<
  HTMLDivElement,
  AgentAudioVisualizerAuraComponentProps
>(function AgentAudioVisualizerAura(
  ...
  const dimension = AURA_SIZE_PX[size ?? 'lg'];
